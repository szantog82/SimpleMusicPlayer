...
public class MainActivity extends AppCompatActivity implements PlaybackService.PlaybackListener, AdapterView.OnItemClickListener, View.OnClickListener, SeekBar.OnSeekBarChangeListener, ListFilesAdapter.OnFileCheckedChangeListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        drawerLayoutListAdapter = new DrawerLayoutListAdapter(this, songCollectionPathsToSend, currentPosition);
        navListView.setAdapter(drawerLayoutListAdapter);
        ...
        ActivityCompat.requestPermissions(MainActivity.this, new String[]{Manifest.permission.READ_EXTERNAL_STORAGE}, 15);
        listDir();

        if (sharedPreferences.getString(getString(R.string.SHARED_LAST_SONGS_ARRAY), null) != null) {
            songCollectionPathsToSend.clear();
            songCollectionPathsToSend.addAll(Arrays.asList(sharedPreferences.getString(getString(R.string.SHARED_LAST_SONGS_ARRAY), null).split(getString(R.string.DIVIDER))));
        }
    ...
    public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
        ...
        if (file.isDirectory()) {
            ...
            for (File f : files) {
                if (f.isFile()) {
                    songCollectionPathsToSend.add(f.getPath());
                }
            }
            FileItem.sortStringArray(songCollectionPathsToSend);
            int index = songCollectionPathsToSend.indexOf(currentDirItems.get(i).getPath());
            int length = songCollectionPathsToSend.size();
            if (index > length / 2) {
                for (int k = 0; k < length / 2; k++) {
                    if (k + length / 2 < length) {
                        String temp = songCollectionPathsToSend.get(k);
            ...
            drawerLayoutListAdapter.notifyDataSetChanged();
            serviceIntent.putStringArrayListExtra(getString(R.string.FILES_TO_PLAY), songCollectionPathsToSend);
